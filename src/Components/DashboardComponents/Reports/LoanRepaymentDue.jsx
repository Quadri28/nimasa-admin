import React, { useEffect, useState, useMemo, useContext } from 'react'
import UnpaginatedTable from './UnpaginatedTable'
import axios from '../../axios'
import { UserContext } from '../../AuthContext'

const LoanRepaymentDue = () => {
  const [products, setProducts] = useState([])
  const [reports, setReports] = useState([])
   const [product, setProduct]= useState('')
   const [startDate, setStartDate]= useState('')
  const [endDate, setEndDate]= useState('')
const {credentials} = useContext(UserContext)
const nodeId= credentials.logInfo.nodeId

const getReports=()=>{
  axios(`Reports/loan-repayment-due-report?NodeId=${nodeId}&ProductCode=${product}&TransactionDateFrom=${startDate}&TransactionDateTo=${endDate}`, {headers:{
      Authorization: `Bearer ${credentials.token}`
  }})
  .then(resp=>{
      if (resp.data.data.loanRepaymentDues) {
      setReports(resp.data.data.loanRepaymentDues)
      }
  })
}
  const getProducts=()=>{
      axios('LoanProduct/loan-products',{headers:{
        Authorization: `Bearer ${credentials.token}`
      }}).then(resp=>setProducts(resp.data.data))
    }
  useEffect(()=>{
      getProducts()
  },[])

  useEffect(()=>{
      getReports()
  },[nodeId, product, startDate, endDate])
  const column=[
      {Header: 'Cust. ID', accessor:'customerId'},
      {Header: 'Loan Acct No', accessor:'accountNumber'},
      {Header: 'Settlement Acct', accessor:'settlementAcct1'},
      {Header: 'Acct. Title', accessor:'fullName'},
      {Header: 'Facility Amt.', accessor: 'facilityAmount', Cell:(({value})=>{
          return <span>{new Intl.NumberFormat('en-US', {minimumFractionDigits:2}).format(value)}</span>
        })},
      {Header: 'Princ. Due', accessor: 'principalAmt', Cell:(({value})=>{
          return <span>{new Intl.NumberFormat('en-US', {minimumFractionDigits:2}).format(value)}</span>
        })},
      {Header: 'Int. Due', accessor:'interestAmt', Cell:(({value})=>{
          return <span>{new Intl.NumberFormat('en-US', {minimumFractionDigits:2}).format(value)}</span>
        })},
      {Header: 'Date Due', accessor:'date_due'},
      {Header: 'Loan Term (Month)', accessor:'term'},
      {Header: 'Date Opened', accessor:'dateOpened'},
      {Header: 'Maturity Date', accessor:'matDate'},
      {Header: 'Created By', accessor:'createdBy'},
  ]

const columns = useMemo(() => column, []);

return (
  <div className='card py-4 px-3 mt-3 rounded-4' style={{border:'solid 1px #fafafa'}}>
    <div className='admin-task-forms mb-3'>
    <div className="d-flex flex-column gap-1">
        <label htmlFor="startDate">
          Start Date<sup className="text-danger">*</sup>
        </label>
        <input
          type="date"
          name={startDate}
          onChange={(e) => setStartDate(e.target.value)}
        />
      </div>
      <div className="d-flex flex-column gap-1">
        <label htmlFor="startDate">
          End Date<sup className="text-danger">*</sup>
        </label>
        <input
          type="date"
          name={endDate}
          onChange={(e) => setEndDate(e.target.value)}
        />
      </div>
      <div className="d-flex flex-column gap-1">
        <label htmlFor="startDate">
          Select product type<sup className="text-danger">*</sup>
        </label>
        <select
          type="date"
          name={endDate}
          onChange={(e) => setProduct(e.target.value)}
        >
          <option value="">Select</option>
          {
              products.map((product)=>(
                  <option value={product.productCode} key={product.productCode}>{product.productName}</option>
              ))
          }
          </select>
      </div>
    </div>
    <div style={{fontSize:'14px'}}>
    <UnpaginatedTable
      data={reports}
      columns={columns}
      filename="LoanRepaymentDueReports.csv"
    />
  </div>
  </div>
)
}

export default LoanRepaymentDue
