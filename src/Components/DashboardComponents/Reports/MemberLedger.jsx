...
const MemberLedger = () => {
  ...
  const getReports = async () => {
    try {
      const resp = await axios.get(
        `Reports/member-ledger-report?AccountNumber=${account}&StartDate=${startDate}&EndDate=${endDate}&ReportOption=${option}&GlobalStatement=${globalStatement}`,
        {
          headers: {
            Authorization: `Bearer ${credentials.token}`,
          },
  ...
  useEffect(() => {
    getReports();
  }, [account, startDate, endDate, option, globalStatement]);
  ...
  return (
    <div className="card mt-3 p-3  rounded-4">
      ...
      <div className="d-flex align-items-center gap-2">
          <label htmlFor="globalStatement">Global Statement</label>
          <input
            type="checkbox"
            name="globalStatement"
            required
            onChange={(e) => setGlobalStatement(e.target.checked)}
          />
        </div>
      </div>
      <div className="admin-task-forms">
              ...
              return (
                <>
                  {ledger?.memberLedgers?.map((led, i) => (
                    <tr key={i}>
                      <td>{new Date(led?.date).toLocaleDateString()} {new Date(led?.valueDate).toLocaleTimeString()}</td>
