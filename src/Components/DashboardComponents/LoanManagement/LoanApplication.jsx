...
const LoanApplication = () => {
  ...
  const getMaturityDate = () => {
    axios(
      `Common/calculate-maturity-date-by-first-payment-date?Value=${term}&FreqCode=${frequency}&FirstPaymentDate=${firstPaymentDate}`,
      {
        headers: {
  ...
  const getGuarantors = () => {
    axios("LoanApplication/get-loan-gaurators", {
      headers: {
        Authorization: `Bearer ${credentials.token}`,
      },
  ...
  const onSubmit = (values) => {
    ...
    setLoading(true);
    axios
      .post("LoanApplication/loan-application", payload, {
        headers: {
          Authorization: `Bearer ${credentials.token}`,
        },
      })
        ...
        >
          ...
          <div className="p-3 admin-task-forms">
            ...
            <div className="d-flex flex-column gap-1">
              ...
              <Field
                name="maturityDate"
                value={new Date(maturityDate).toLocaleDateString("en-CA")}
              />
            </div>
          ...
          <div className="admin-task-forms px-3 mt-1 mb-2">
            <div>
              <label htmlFor="">
                Search guarantor's name<sup className="text-danger">*</sup>{" "}
              </label>
              <Multiselect
                data={guarantors}
