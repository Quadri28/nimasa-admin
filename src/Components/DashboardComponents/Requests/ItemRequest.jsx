...
const ItemRequest = () => {
  ...
  const getAccounts = async () => {
    await axios("RequestVerification/get-debit-and-credit-account-numbers", {
      headers: {
  ...
  const getMemberSavingsAccount = async () => {
    await axios(
      `RequestVerification/get-member-savings-account-numbers-for-item-request-purchased?username=${request?.username}`,
      {
        headers: {
          Authorization: `Bearer ${credentials.token}`,
        },
  ...
  const savingAccountDetails = savings.find(
    (saving) => saving.accountNumber === request?.accountnumber,
  );
    ...
    },
    {
      ...
      Cell: ({ value }) => {
        if (value === "Approved") {
          ...
          return (
            <div
              className="suspended-status px-3"
              style={{ width: "max-content" }}
            >
              <hr />
              <span>{value}</span>
          ...
          return (
            <div
              className="sold-status px-3 white"
              style={{ width: "max-content" }}
            >
              <hr />
              <span>Sold</span>
  ...
  const uploadRequest = (e) => {
    ...
    const payload = {
      ...
      debitAccount:
        request.paymentOption === "Savings Account"
          ? savingAccountDetails.accountNumber
          : input.debitAccount,
      narration: `Payment for item bought-${request?.orderId}`,
      loanProduct:
        request.paymentOption === "Pay At Banks" ? "" : input.product,
    };
    axios
      .post("RequestVerification/post-item-request-purchased", payload, {
        headers: {
          Authorization: `Bearer ${credentials.token}`,
        },
      })
  ...
  return (
    <div>
      ...
      >
        <div>
          <h3 style={{ fontSize: "20px", fontWeight: "600" }} className="mt-1">
            View Item Request
          </h3>
          <div
            className="d-flex gap-3 mt-3 align-items-center flex-wrap"
                ...
                {requests?.map((request) => (
                  <tr key={request.id}>
                    ...
                    <td>
                      {new Intl.NumberFormat("en-US", {
                        minimumFractionDigits: 2,
                      }).format(request?.unitPrice)}
                    </td>
                    <td>
                      {new Intl.NumberFormat("en-US", {
                        minimumFractionDigits: 2,
                      }).format(request?.subTotal)}
                    </td>
          ...
          <div className="d-flex gap-3 mt-5 justify-content-end">
            <form onSubmit={approveRequest}>
              <button
                disabled={loading}
                className="border-0 btn-md member"
                style={{ fontSize: "14px" }}
              >
                Approve
              </button>
            ...
            {!isReject ? (
              <button
                className="border-0 rounded-5 btn-md px-3"
              ...
              <button
                disabled={loading}
                className="border-0 px-3 btn-md rounded-5"
      ...
      >
        ...
        <div>
          ...
          <div className="mt-4">
            <div className="d-flex flex-wrap flex-column gap-3 discourse">
              <div
                    ...
                    >
                      ...
                      {request?.paymentOption !== "Pay At Banks" && (
                        <span>
                          ...
                          {new Intl.NumberFormat("en-US", {
                            minimumFractionDigits: 2,
                          }).format(savingAccountDetails?.accountBalance)}
                        </span>
                      )}
                    </div>
                  </>
              ...
              <div className="d-flex gap-2 align-items-center p-2 mt-3 discourse">
          ...
          <form onSubmit={uploadRequest}>
            <div className="d-flex flex-column gap-2 mt-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  style={{ border: "solid 1px #f2f2f2" }}
                  name="narration"
                  value={`Payment for item bought-${request?.orderId}`}
                  onChange={handleChange}
                />
              </div>
            ...
            <div className="d-flex justify-content-end">
              <button type="submit" className="border-0 btn-md mt-3 member">
                Submit
              </button>
            </div>
          </form>
