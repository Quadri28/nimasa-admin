...
const LoanRepaymentRequest = () => {
  ...
  const getReports = () => {
    axios("RequestVerification/loan-repayment-requests", {
      headers: {
  ...
  const getRequest = () => {
    axios(`RequestVerification/loan-repayment-request?id=${id}`, {
      headers: {
        Authorization: `Bearer ${credentials.token}`,
      },
  ...
  const postAccount = (e) => {
    ...
    axios
      .post("RequestVerification/loan-repayment-request-approved", payload, {
        headers: {
          Authorization: `Bearer ${credentials.token}`,
        },
      })
  ...
  const rejectAccount = (e) => {
    ...
    axios
      .post("RequestVerification/loan-repayment-request-reject", payload, {
        headers: {
          Authorization: `Bearer ${credentials.token}`,
        },
      })
  ...
  return (
    <div>
      <UnpaginatedTable
        data={reports}
        filename="LoanRepaymentRequest.csv"
        columns={columns}
      />
      <Modal
        isOpen={isOpen}
      ...
      >
        ...
        <div className="d-flex flex-column gap-3 mt-3">
          <div className="d-flex gap-3 align-items-center flex-wrap discourse">
            <div>
            <span>Member ID: </span> <span>{request?.memberNo}</span>
            </div>
             <div>
              <span>Full Name:</span> <span>{request?.memberName}</span>
            </div>
            <img src={request?.profileImage} alt="profile image" className="view-img img-fluid"/>
          </div>
          <div
...
<span>Transaction Date: </span>{" "}
           ...
           <div
            ...
            <span>
              {new Intl.NumberFormat("en-US", {
                minimumFractionDigits: 2,
              }).format(request?.principalAmount)}
            </span>
          </div>
          </div>
          ...
          <div className="d-flex gap-3">
            ...
            <span>
              {new Intl.NumberFormat("en-US", {
                minimumFractionDigits: 2,
              }).format(request?.totalRepayAmount)}
            </span>
          </div>
          ...
          <div className="d-flex gap-3" style={{ fontSize: "14px" }}>
            ...
            <span>
              ...
              {new Intl.NumberFormat("en-US", {
                minimumFractionDigits: 2,
              }).format(request?.balance)}
            </span>
          </div>
          </div>
        </div>
        <h3 style={{ fontSize: "18px", fontWeight: "600", marginBlock: "1.5rem" }}>
          Loan Schedules
        </h3>
        <div className="">
          ...
          <div className="d-flex flex-column gap-3 mt-3">
            <select
              ...
              className="w-100 mb-2 p-2 rounded-3"
              style={{
            ...
            <div className="d-flex gap-3 justify-content-end">
              <button
                className="btn-md member px-4 border-0"
                onClick={postAccount}
              >
                Post
              </button>
